...
public class LdapBackendTestNewStyleConfig2 {
    ...
    public void testLdapAuthenticationSSLSSLv3() throws Exception {
        ...
        try {
            ...
            var message = CryptoServicesRegistrar.isInApprovedOnlyMode() ? "'protocols' cannot be null, or contain unsupported protocols" : "No appropriate protocol";
    ...
    public void testLdapAuthenticationSSLUnknownCipher() throws Exception {
        ...
        try {
            ...
            assertThat(e.getCause().getClass(), is(org.ldaptive.provider.ConnectionException.class));
            var message = CryptoServicesRegistrar.isInApprovedOnlyMode() ? "No usable cipher suites enabled" : "Unsupported CipherSuite: AAA";
            Assert.assertTrue(ExceptionUtils.getStackTrace(e).contains(message));
        }
